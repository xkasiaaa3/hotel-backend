...
class AuthIntegrationSpec extends Specification implements UserSample{
    ...
    def "Guest should register on new email"() {
        ...
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post("/api/auth/register").content(om.writeValueAsString(REGISTER_REQUEST))).andReturn()
    ...
    def "Guest should log in on existing account with good credentials"() {
        ...
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post("/api/auth/authenticate").content(om.writeValueAsString(AUTHENTICATION_REQUEST))).andReturn()
    ...
    def "Guest shouldn't log in on existing account with bad credentials"() {
        ...
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post("/api/auth/authenticate").content(om.writeValueAsString(AuthenticationRequest.builder().email(USER_EMAIL).password("123").build()))).andReturn()
